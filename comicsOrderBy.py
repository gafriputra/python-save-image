...
load_dotenv()
'''
Script Untuk Menjalankan SAVE IMAGE berdasarkan ORDER BY (nama kolom) DESC, LIMIT, dan OFFSET

python comicsOrderBy.py nama_kolom limit offset
'''
orderBy = str(sys.argv[1])
limit = str(sys.argv[2])
offset = str(sys.argv[3])
...
cursor = conn.cursor()
queryGetComic = "SELECT id FROM comics WHERE deleted_at is null ORDER BY " + orderBy + " DESC LIMIT " + limit + " OFFSET " + offset
cursor.execute(queryGetComic)
getComic = cursor.fetchall()
getComic = np.array(getComic)
idComic = []
for id in getComic:
    idComic.append(id[0])
newIdComic = ",".join(str(x) for x in idComic)
quuery = """SELECT chapters.id, chapters.chapter_title, chapters.content, comics.title 
                FROM chapters 
                INNER JOIN comics ON chapters.comic_id = comics.id 
                WHERE chapters.content IS NOT NULL 
                    AND chapters.content NOT LIKE '%img.y7349.top%' 
                    AND chapters.content not like '' 
                    AND comic_id in (""" + str(newIdComic) + ")"
print(quuery)
# Execute Query
