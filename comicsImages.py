...
from saveToLocal import save_image_comic_images
from dotenv import load_dotenv

runningFor = str(sys.argv[1])
value = str(sys.argv[2])
load_dotenv()
'''
Script Untuk Menjalankan SAVE semua asset gambar di tabel comics

python comicsImages.py
'''
...
if runningFor == 'all':
    query = """ SELECT id, title, thumbnail, banner, previews
                FROM comics 
...
with concurrent.futures.ThreadPoolExecutor(max_workers=None) as executor:
    ...
    future_to_args = {executor.submit(save_image_comic_images, image_arg): image_arg for image_arg in image_args}
